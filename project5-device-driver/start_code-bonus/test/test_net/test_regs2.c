...
static void send_desc_init(mac_t *mac)
{
    ...
    {
        send_desc[i].tdes0 = 0;
        send_desc[i].tdes1 = DescTxLast | DescTxFirst | TxDescChain | BUFFSIZE;
        send_desc[i].tdes2 = TxBuff & 0x1fffffff;
        send_desc[i].tdes3 = ((uint32_t)&send_desc[i+1]) & 0x1fffffff;
        memcpy((uint8_t *)TxBuff, buffer, sizeof(buffer));

        TxBuff += BUFFSIZE;
    }

    send_desc[63].tdes0 = 0;
    send_desc[63].tdes1 = DescTxLast | DescTxFirst | TxDescChain | TxDescEndOfRing | BUFFSIZE;
    send_desc[63].tdes2 = TxBuff & 0x1fffffff;
    send_desc[63].tdes3 = ((uint32_t)&send_desc[0]) & 0x1fffffff;
    memcpy((uint8_t *)TxBuff, buffer, sizeof(buffer));
...
static void recv_desc_init(mac_t *mac)
{
    ...
    for(i = 0; i < 63; i++)
    {
        ...
        receive_desc[i].tdes1 = RxDescChain | BUFFSIZE;
        receive_desc[i].tdes2 = RxBuff & 0x1fffffff;
        receive_desc[i].tdes3 = ((uint32_t)&receive_desc[i+1]) & 0x1fffffff;

        RxBuff += BUFFSIZE;
    }

    receive_desc[63].tdes0 = 0;
    receive_desc[63].tdes1 = RxDescChain | RxDescEndOfRing | BUFFSIZE;
...
void phy_regs_task2()
{
    ...
    recv_flag[ch_flag] = (uint32_t)Recv_desc;
