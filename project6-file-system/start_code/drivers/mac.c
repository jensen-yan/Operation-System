...
void irq_mac(void)
{
    ...
    desc_t *receive = (desc_t *)recv_flag[0];
    clear_interrupt();
    /*int i;
    for(i = 0; i < 4096; i++)
    {
...
{
    reg_write_32(0xbfd0105c, 0x8);//reg_read_32(0xbfd0105c) | 0x8);
}
...
void check_recv(mac_t *test_mac)
{
    ...
    printk(">>Receive %d Valid Packages\n",ch_flag);;
}
...
void do_net_send(uint32_t td, uint32_t td_phy)
{
    ...
    reg_write_32(DMA_BASE_ADDR + 0x18, reg_read_32(DMA_BASE_ADDR + 0x18) | 0x02202000); // start tx, rx
    reg_write_32(DMA_BASE_ADDR + 0x1c, 0x10001 | (1 << 6));
