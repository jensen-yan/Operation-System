...
void do_spawn(task_info_t *task)
{
    ...
    bzero(&new_pcb->pgtab, sizeof(new_pcb->pgtab));

	// stack 
	new_pcb->kernel_stack_top = new_pcb->kernel_context.regs[29] = new_stack();
    new_pcb->user_stack_top = new_pcb->user_context.regs[29] = 0x1000 - 4;
    ...
    new_pcb->kernel_context.hi = new_pcb->user_context.hi = new_pcb->pid;
    new_pcb->my_queue = &ready_queue;

    stack_Refill(0, new_pcb->pid, new_pcb);
